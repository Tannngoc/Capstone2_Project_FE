  ...
  return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '2rem', backgroundColor: 'white', borderRadius: '4px', margin: '2rem 0' }}>
        <Box sx={{display: 'flex', gap:'.5rem'}}>
          <Button sx={{color:mode === 'tipsrank' ? 'red' : 'black', border:mode === 'tipsrank' ? '2px solid red' : '2px solid black'}} onClick={() => handleModeChange(null, 'tipsrank')}>
            TipRanks
          </Button>
          <Button sx={{color:mode === 'release' ? 'red' : 'black', border:mode === 'release' ? '2px solid red' : '2px solid black'}} onClick={() => handleModeChange(null, 'release')}>
            Release
          </Button>
        </Box>
        ...
        <Box sx={{display:'flex', justifyContent:'space-between'}}>
          ...
          {data[mode].map((item, index) => (
            <ListItem key={index}>
              <ListItemText sx={{borderBottom:'1px solid #9f9f9f', opacity:'.8'}} primary={`${item.day} at ${item.time} - ${item.description}`} />
            </ListItem>
          ))}
        </List>
        </Box>
