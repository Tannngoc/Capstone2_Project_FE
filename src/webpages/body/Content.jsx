...
// Remove duplicate/unused imports and fix Qoute typo
// import Qoute from "./components/Quote"; // Not used
...
const Content = ({ isLogin }) => {
  ...
  const [showChat, setShowChat] = useState(false); // Fix: define showChat state
    ...
    try {
      ...
      setStockData((prev) => ({
        ...
        [symbol]: {
          ...
          financials: null,
          forecasts: null,
          news: null, // Fix: add news property to avoid undefined
        },
      }));
    } catch (err) {
  ...
  if (companies.every((symbol) => stockData[symbol]?.chartData)) {
    ...
    ["open", "close", "high", "low"].forEach((key) => {
      const selected = calculateAverages(
        stockData[selectedCompany].chartData
      )[key];
    ...
    >
      <Box sx={{ width: "90vw" }}>
        ...
        <Box>
          ...
          {/* Fix: Use correct Quote component and pass correct props */}
          <Quote isLogin={isLogin} selectedCompany={selectedCompany} />
        </Box>
      </Box>
    </Box>
  );
